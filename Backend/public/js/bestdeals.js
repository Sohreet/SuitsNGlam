...
async function loadCartCount() {
  ...
  if (!user?.email) return badge.textContent = "";
  ...
  col.innerHTML = `
    <div class="card shadow-sm card-button">
      ...
      <div class="card-body">
        ...
        <button class="btn btn-primary w-100" onclick="addDeal('${p._id}')">Add to Cart</button>
      </div>
    </div>
  `;
...
function meterChange(id, amount) {
  ...
  val = Math.max(1, Math.min(50, val + amount));
  field.value = val;
}
