  ...
  col.innerHTML = `
    <div class="card shadow-sm card-button">
      ...
      <div class="card-body">
        ...
        <button class="btn btn-primary w-100" onclick="addNewArrival('${p._id}')">Add to Cart</button>
      </div>
    </div>
  `;
...
function meterChange(id, amt) {
  ...
  v = Math.max(1, Math.min(50, v + amt));
  field.value = v;
}
...
// ===============================
async function addNewArrival(id) {
  const user = JSON.parse(localStorage.getItem("sg_user"));
